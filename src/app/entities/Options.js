...
export const changeKeyMapping = (mode, key, keyCode) => u({
...
export const changePanelPlacement = (placement) => u({ 'player.P1.panel': placement })

// Lane cover
export const laneCover = (state) => {
  return Math.min(50, Math.max(-50, Math.round(state['player.P1.lane-cover'] * 100))) / 100 || 0
}
export const changeLaneCover = (laneCover) => u({ 'player.P1.lane-cover': laneCover })
