...
export function gameStart(song, chart) {
  ga('send', 'song', 'play', song.title)
  ga('send', 'game', 'start', getLabel(chart), chart.info.level)
}

export function gameFinish(song, chart, gameState) {
  let state = gameState.player(gameState.game.players[0])
  ga('send', 'song', 'finish', song.title)
  ga('send', 'game', 'finish', getLabel(chart), state.stats.score)
}
